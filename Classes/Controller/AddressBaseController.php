...
class AddressBaseController extends ActionController
{
    ...
    private function handleKnownExceptionsElseThrowAgain(\Exception $exception): ResponseInterface
    {
        ...
        if (
            ...
            $this->handleNoAddressFoundError($this->settings['detail']['errorHandling']);
        } else {
            throw $exception;
        }
    }
    ...
    protected function handleNoAddressFoundError($configuration): ?string
    {
        ...
        switch ($configuration[0]) {
            ...
            case 'pageNotFoundHandler':
                $GLOBALS['TSFE']->pageNotFoundAndExit('No address found.');
                break;
    ...
    protected function emitActionSignal($classPart, $signalName, array $signalArguments)
    {
        ...
        return $this->signalSlotDispatcher->dispatch('WapplerSystems\\Address\\Controller\\' . $classPart, $signalName,
            $signalArguments);
    }
}
