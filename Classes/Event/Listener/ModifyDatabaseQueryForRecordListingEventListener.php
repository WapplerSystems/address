...
final class ModifyDatabaseQueryForRecordListingEventListener
{
    ...
    public function modify(ModifyDatabaseQueryForRecordListingEvent $event): void
    {
        if ($event->getTable() === 'tt_content' && $event->getPageId() > 0) {
            ...
            $pageRecord = BackendUtility::getRecord('pages', $event->getPageId(), 'uid', " AND doktype='254' AND module='address'");
            if (is_array($pageRecord)) {
                $tsConfig = BackendUtility::getPagesTSconfig($event->getPageId());

                if ((int)($tsConfig['tx_address.']['showContentElementsInAddressSysFolder'] ?? 0)  === 1) {
                    return;
                }
     ...
     * that no elements belonging to address articles
    ...
    protected function addFlashMessage(): void
    {
        $message = GeneralUtility::makeInstance(
            FlashMessage::class,
            $this->getLanguageService()->sL('LLL:EXT:address/Resources/Private/Language/locallang_be.xlf:hiddenContentElements.description'),
            '',
