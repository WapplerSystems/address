...
class TemplateLayout implements SingletonInterface
{
    ...
    public function getAvailableTemplateLayouts($pageUid)
    {
        ...
        if (isset($GLOBALS['TYPO3_CONF_VARS']['EXT']['address']['templateLayouts'])
            && is_array($GLOBALS['TYPO3_CONF_VARS']['EXT']['address']['templateLayouts'])
        ) {
            $templateLayouts = $GLOBALS['TYPO3_CONF_VARS']['EXT']['address']['templateLayouts'];
        }
    ...
    protected function getTemplateLayoutsFromTsConfig($pageUid)
    {
        ...
        if (isset($pagesTsConfig['tx_address.']['templateLayouts.']) && is_array($pagesTsConfig['tx_address.']['templateLayouts.'])) {
            $templateLayouts = $pagesTsConfig['tx_address.']['templateLayouts.'];
        }
        return $templateLayouts;
    }
}
